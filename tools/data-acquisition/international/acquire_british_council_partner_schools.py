#!/usr/bin/env python3
"""Acquire the current British Council Egypt Partner Schools contact list.
...
This adapter:
- downloads the public September 2026 PDF;
- extracts the ruled four-column tables with pdfplumber;
- preserves school/institution name, address, phones and contact text as a
  source-shaped supporting snapshot;
- never creates canonical identities, never auto-merges and never publishes.

Important policy boundary: Partner School / attached-centre status is supporting
exam-delivery evidence only. Each candidate still requires stronger eligibility
and official-institution evidence before canonical promotion.
"""
...
def extract_rows(pdf_bytes: bytes) -> tuple[list[dict], dict]:
        ...
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            for table in tables:
                ...
                for raw_row in table:
                    ...
                    if not name:
                        continue
                    # Remove obvious non-row artifacts if table extraction catches a title/footer.
    ...
    # Preserve multiple campuses/entries when the address differs, but remove exact
    # extraction duplicates generated by overlapping table detection.
    deduped: list[dict] = []
...
def acquire(output_dir: Path, timeout: float) -> dict:
    ...
    rows, meta = extract_rows(response.content)
    if not rows:
...
def main() -> int:
    ...
    acquire(args.output_dir, args.timeout)
    return 0
