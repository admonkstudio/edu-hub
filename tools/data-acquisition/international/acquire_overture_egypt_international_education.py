...
Because that rectangle also touches neighbouring countries, rows are accepted
only when Overture's own address data contains country code EG.

Rows are retained as source candidates and never grant eligibility, create
identities, merge records, mutate a database or create public output.
"""
...
def address_countries(addresses: object) -> set[str]:
    countries: set[str] = set()
    if isinstance(addresses, (list, tuple)):
        for item in addresses:
            if isinstance(item, dict):
                country = compact(item.get("country"))
                if country:
...
def build(output: Path) -> dict:
    ...
    bbox_candidate_rows = len(rows)
    non_egypt_rows_rejected = 0
    records: list[dict] = []
    seen_ids = set()
    for values in rows:
        row = dict(zip(columns, values))
        countries = address_countries(row.get("addresses"))
        if "EG" not in countries:
            non_egypt_rows_rejected += 1
            continue
    ...
    result = {
        ...
        "country_filter": "EG",
        "bbox_candidate_rows": bbox_candidate_rows,
        "non_egypt_rows_rejected": non_egypt_rows_rejected,
