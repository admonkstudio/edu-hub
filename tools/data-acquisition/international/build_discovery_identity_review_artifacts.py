...
DEFAULT_DECISIONS = HERE / "seeds" / "discovered-identity-decisions-2026-09-15-batch1.json"
...
def build(base_dir: Path, qualified_leads_path: Path, decisions_path: Path, output_dir: Path) -> dict:
    ...
    qualified_leads = load_jsonl(qualified_leads_path)
    ...
    if len(qualified_leads) != 35:
        raise AssertionError("expected 35 British Council discovery rows after primary scope review")

    decision_rows = decisions.get("records") or []
    if decisions.get("records_count") != len(decision_rows) or len(decision_rows) != 2:
        raise AssertionError("first discovered identity review batch must contain exactly two decisions")
    ...
    lead_by_key = {(row["source_id"], row["source_record_id"]): row for row in qualified_leads}
    if len(lead_by_key) != len(qualified_leads):
        raise AssertionError("duplicate discovery source key in qualified-lead input")
    ...
    for decision in decision_rows:
        ...
        draft = {
            ...
            "review_batch_id": decisions["batch_id"],
    ...
    unresolved_discovery = [
        row for row in qualified_leads
        if row.get("scope_state") != "eligible"
    ]
    qualified_not_identity_reviewed = [
        row for row in qualified_leads
        if row.get("scope_state") == "eligible"
        and (row["source_id"], row["source_record_id"]) not in seen_discovery_source_keys
    ]
    if len(unresolved_discovery) != 33:
        raise AssertionError(f"expected 33 unresolved British Council discovery candidates, found {len(unresolved_discovery)}")
    if qualified_not_identity_reviewed:
        raise AssertionError("every currently qualified discovery row must be explicitly handled by this identity batch")
    ...
    write_jsonl(discovery_queue_path, unresolved_discovery)
    ...
    summary = {
        ...
        "unqualified_discovery_rows": len(unresolved_discovery),
...
def main() -> int:
    ...
    build(args.base_dir, args.qualified_leads, args.decisions, args.output_dir)
    return 0
